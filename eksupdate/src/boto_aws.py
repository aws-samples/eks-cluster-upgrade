...
def wait_for_ready(instanceid,regionName):
    ...
    try:
        while ec2_client.describe_instance_status(InstanceIds=[instanceid])['InstanceStatuses'][0]['InstanceStatus']['Details'][0]['Status']!='passed':
            end = time.time()
            hours, rem = divmod(end-st, 3600)
            minutes, seconds = divmod(rem, 60)
            print(instanceid," waiting for the instance to pass the Health Checks ","{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
...
def add_node(asg_name,regionName):
    ...
    if int(old_capacity_des) >= int(old_capacity_mx):
        asg_client.update_auto_scaling_group(
            ...
            MaxSize=(int(old_capacity_mx)+int(old_capacity_des))
        )
...
def get_num_of_instances(asg_name, exclude_ids,regionName):
    ...
    instance_ids = [instance["InstanceId"] for instance in response["AutoScalingGroups"]
                    [0]["Instances"] if instance["InstanceId"] not in exclude_ids]
    response = ec2_client.describe_instances(InstanceIds=instance_ids)
    for reservation in response["Reservations"]:
        for instance in reservation["Instances"]:
            instances.append(instance)
    ''' getting the instance in running or pending state'''
    instances = [instance for instance in instances if instance["State"]
                 ["Name"] in ["running", "pending"]]
...
def addAutoScaling(asg_name, img_id,regionName):
    ...
    # print(response)
    sourceInstanceId = response.get('AutoScalingGroups')[
