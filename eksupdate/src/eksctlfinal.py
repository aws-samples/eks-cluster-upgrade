...
def upgrade_cluster(region,clust_name,d,bclient,version):
    ...
    args='~/eksctl upgrade cluster --name='+clust_name+' --version '+version+' --approve'           #upgrades cluster to one version above 
    ...
    logs_pusher(regionName=region,cluster_name=clust_name,msg="The time Taken For the cluster Upgrade "+"{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
...
def create_managed_nodegroup(region,clust_name,client):
    start_time=time.ctime()
    start=time.time()
    logs_pusher(regionName=region,cluster_name=clust_name,msg="creation of managed nodegroups Started At "+str(start_time))
    ...
    hours, rem = divmod(end-start, 3600)
    minutes, seconds = divmod(rem, 60)
    print("The time Taken For the creation of managed nodegroups ","{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
    logs_pusher(regionName=region,cluster_name=clust_name,msg="The time Taken For the creation of managed nodegroups  "+"{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
...
def create_unmanaged_nodegroup(region,clust_name,client):
    start_time=time.ctime()
    start=time.time()
    logs_pusher(regionName=region,cluster_name=clust_name,msg="creation of Unmanaged nodegroups Started At "+str(start_time))
    ...
    hours, rem = divmod(end-start, 3600)
    minutes, seconds = divmod(rem, 60)
    print("The time Taken For the creation of Unmanaged nodegroups ","{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
    logs_pusher(regionName=region,cluster_name=clust_name,msg="The time Taken For the creation of Unmanaged nodegroups  "+"{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
...
def update_managed_nodegroup(region,clust_name,version,d,bclient):
    ...
    start_time=time.ctime()
    start=time.time()
    logs_pusher(regionName=region,cluster_name=clust_name,msg="Updation of managed nodegroups Started At "+str(start_time))
    ...
    if(response['nodegroups']!=[ ]):                                        #to verify if MANAGED NODEGROUP exists
        for i in response['nodegroups']:
            ...
            args="~/eksctl upgrade nodegroup --name="+i+" --cluster="+clust_name+' --kubernetes-version='+version     #UPDATES MANAGED NODEGROUP
            output=subprocess.call(args,shell=True)
            response=bclient.describe_nodegroup(clusterName=clust_name,nodegroupName=i)
    ...
    hours, rem = divmod(end-start, 3600)
    minutes, seconds = divmod(rem, 60)
    print("The time Taken For the Updation of managed nodegroups ","{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
    logs_pusher(regionName=region,cluster_name=clust_name,msg="The time Taken For the Updation of managed nodegroups  "+"{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
