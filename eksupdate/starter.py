...
class StatsWorker(threading.Thread):
    ...
    def run(self):
        while self.queue.not_empty:
            ...
            if typse=="managed":
                start= time.time()
                logs_pusher(regionName=regionName,cluster_name=cluster_name,msg="Updating Node Group {ng} To version {versi}".format(ng=ng_name,versi=to_update))
                ...
                print("Updated Node Group {ng} To version {versi} ".format(ng=ng_name,versi=to_update),"{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
                logs_pusher(regionName=regionName,cluster_name=cluster_name,msg="Updated Node Group {ng} To version {versi}".format(ng=ng_name,versi=to_update))
            
                self.queue.task_done()
            elif typse=="selfmanaged":
                start= time.time()
                logs_pusher(regionName=regionName,cluster_name=cluster_name,msg="Updating Node Group {ng} To version {versi}".format(ng=ng_name,versi=to_update))
                out=actual_update(cluster_name=cluster_name,asg_iter=ng_name,to_update=to_update,regionName=regionName,max_retry=max_retry,forced=forced)
                end = time.time()
                hours, rem = divmod(end-start, 3600)
                minutes, seconds = divmod(rem, 60)
                print("Updated Node Group {ng} To version {versi} ".format(ng=ng_name,versi=to_update),"{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
