...
def log_pusher(log_details,cluster_name,region,msg):
    ...
    response = logs.put_log_events(logGroupName = log_group,logStreamName=log_stream,logEvents = [{'timestamp':timestamp,'message':msg}], sequenceToken=response['logStreams'][0]['uploadSequenceToken'])
...
def create_log_group_stream(cluster_name,region) :
    ...
    if len(logs.describe_log_streams(logGroupName=log_group,logStreamNamePrefix=log_stream)['logStreams']) > 0 :
        ...
        timestamp = int(round(time.time() * 1000))
        response = logs.put_log_events(logGroupName = log_group,logStreamName=log_stream,logEvents = [{'timestamp':timestamp,'message':'Log stream create at ' + str(timestamp)}])
...
def pod_security_policies(log_details,errors,cluster_name,region,report,customer_report):
    ...
    try :
        ...
        log_pusher(log_details,cluster_name,region,'Some error occured while checking for the policy security policies {err}'.format(err=e))
...
def check_pods_running(addon,log_details,cluster_name,region,report,errors,namespace="kube-system"):
    try:
        ...
        for pod in rep.items:
            if addon in pod.metadata.name :
                ...
                if (pod.status.phase == "Running"):
                    ...
                    log_pusher(log_details,cluster_name,region,addon + " Pod is not running, it is in " + pod.status.phase)
        ...
        log_pusher(log_details,cluster_name,region,'Some error occured while checking for addon pods to be running {err}'.format(err=e))
...
def horizontal_auto_scaler(log_details,errors,cluster_name,region,report,customer_report):
    ...
    try : 
        ...
        log_pusher(log_details,cluster_name,region,'Error occured while checking for horizontal autoscaler {err}'.format(err=e))
...
def security_group_check(log_details,errors,cluster_name,region,cluster,report,customer_report):
    ...
    try : 
        ...
        if len(security_groups)== 0:
            ...
            for s in security_groups:
                try : 
                    ...
                    customer_report['security group'] = ("Security Group " + security_group.id + " is present in VPC with ID" + security_group.vpc_id)
                    log_pusher(log_details,cluster_name,region,"Security Group " + security_group.id + " is present in VPC with ID " + security_group.vpc_id)
        ...
        log_pusher(log_details,cluster_name,region,' Error retireving security  group of cluster {err}'.format(err=e))
...
def iscustomami(node_type,Presentversion,image_id,region):
    ...
    if node_type=="Amazon Linux 2":
        filters = [{'Name':'owner-id','Values':['602401143452']},{'Name': 'name', 'Values': ["amazon-eks-node-{version}*".format(version=Presentversion)]},{'Name':'is-public','Values':['true']}]
    elif "ubuntu" in node_type.lower():
        filters = [{'Name':'owner-id','Values':['099720109477']},{'Name': 'name', 'Values': ["ubuntu-eks/k8s_{version}*".format(version=Presentversion)]},{'Name':'is-public','Values':['true']}]
    elif "bottlerocket" in node_type.lower():
        filters = [{'Name':'owner-id','Values':['092701018921']},{'Name': 'name', 'Values': ["bottlerocket-aws-k8s-{version}*".format(version=Presentversion)]},{'Name':'is-public','Values':['true']}]
    elif "windows" in node_type.lower():
         filters = [{'Name':'owner-id','Values':['801119661308']},{'Name': 'name', 'Values': ["Windows_Server-*-English-*-EKS_Optimized-{version}*".format(version=Presentversion)]},{'Name':'is-public','Values':['true']}]
    else:
        return True
...
def send_email(preflight,log_details,errors,cluster_name,region,report,customer_report,email):
    try :
        ...
        htmlend = ''
        if preflight :
            htmlStart = htmlStart  + '<h2>Preflight check for cluster ' +  cluster_name + ' in region ' +  region + ' : ' + str(report['preflight_status'])+ '</h2>'
        else :
            htmlStart = htmlStart  + '<h2>Postflight check for cluster ' +  cluster_name + ' in region ' +  region + ' : ' + str(report['preflight_status'])+ '</h2>'
        if report['preflight_status']:
            ...
            if preflight :
                htmlStart = htmlStart + '<h3>The preflight check for your cluster failed, please check the below report for more details</h3>'
            else :
                htmlStart = htmlStart + '<h3>The postflight check for your cluster failed, please check the below report for more details</h3>'
                htmlStart = htmlStart + 'Certain things couldn’t be upgraded during the upgrade'

        htmlStart  = htmlStart + '<h3>General Check details</h3><table><thead><tr><th>Name</th><th>Status</th></tr></thead><tbody>'
        ...
        htmlStart = htmlStart + '<tr><td>cluster autoscaler</td><td>' + customer_report['cluster autoscaler']+'</td></tr>'
        if 'cluster upgradation' in customer_report.keys() :
            htmlStart = htmlStart + '<tr><td>cluster upgradation</td><td>' + customer_report['cluster upgradation']+'</td></tr>'
        ...
        if 'depricated Api' in customer_report.keys() :
            htmlStart = htmlStart + '<tr><td>depricated Api</td><td>' + str(customer_report['depricated Api'])+'</td></tr>'
        htmlStart = htmlStart + '<tr><td>horizontal auto scaler</td><td>' + customer_report['horizontal auto scaler']+'</td></tr>'
        htmlStart = htmlStart + '<tr><td>pod disruption budget</td><td>' + str(customer_report['pod disruption budget'])+'</td></tr>'
        htmlStart = htmlStart + '<tr><td>pod security policy</td><td>' + customer_report['pod security policy']+'</td></tr>'
        ...
        if 'coredns' in customer_report['addons'].keys() and len(customer_report['addons']['coredns'].keys()) != 0 :
            if 'image' in customer_report['addons']['coredns'].keys():
                htmlStart = htmlStart + '<p>Core DNS </p>' + '<table><thead><td>Corefile</td><td>Image</td><td>Mount Path</td><td>Version</td></thead><tbody><tr>'
                ...
                if 'default-corefile-fields' in customer_report['addons']['coredns']['corefile'].keys() :
                    htmlStart = htmlStart + '<li>default-corefile-fields ' + str(customer_report['addons']['coredns']['corefile']['default-corefile-fields']) + '</li>'
                if 'userdefined-corefile-fields' in customer_report['addons']['coredns']['corefile'].keys() :
                    htmlStart = htmlStart + '<li>userdefined-corefile-fields ' + str(customer_report['addons']['coredns']['corefile']['userdefined-corefile-fields']) + '</li>'
                ...
                if 'default-mountpaths' in customer_report['addons']['coredns']['mount_paths'].keys() :
                    htmlStart = htmlStart + '<li>default-mountpaths ' + str(customer_report['addons']['coredns']['mount_paths']['default-mountpaths']) + '</li>'
                if 'userdefined-mountpaths' in customer_report['addons']['coredns']['mount_paths'].keys() :
                    htmlStart = htmlStart + '<li>userdefined-mountpaths ' + str(customer_report['addons']['coredns']['mount_paths']['userdefined-mountpaths']) + '</li>'
        ...
        if 'kube-proxy' in customer_report['addons'].keys() and len(customer_report['addons']['kube-proxy'].keys()) != 0 :
            if 'image' in customer_report['addons']['kube-proxy'].keys():
                htmlStart = htmlStart + '<p>Kube Proxy </p>' + '<table><thead><td>Certificate Authority</td><td>Image</td><td>Mount Path</td><td>Version</td><td>Server Endpoint</td></thead><tbody><tr>'
                htmlStart = htmlStart + '<td>' + customer_report['addons']['kube-proxy']['certificate-authority']['message']  + '</td>'
                htmlStart = htmlStart + '<td>' + customer_report['addons']['kube-proxy']['image']  + '</td>'
                htmlStart = htmlStart + '<td>' + customer_report['addons']['kube-proxy']['mount_paths']['message']  + '<ul>'
                if 'default-mountpaths' in customer_report['addons']['kube-proxy']['mount_paths'].keys() :
                    htmlStart = htmlStart + '<li>default-mountpaths ' + str(customer_report['addons']['kube-proxy']['mount_paths']['default-mountpaths']) + '</li>'
                if 'userdefined-mountpaths' in customer_report['addons']['kube-proxy']['mount_paths'].keys() :
                    htmlStart = htmlStart + '<li>userdefined-mountpaths ' + str(customer_report['addons']['kube-proxy']['mount_paths']['userdefined-mountpaths']) + '</li>'
                htmlStart = htmlStart + '</ul></td><td>' + customer_report['addons']['kube-proxy']['version']  + '</td>'
                htmlStart = htmlStart + '<td>' + customer_report['addons']['kube-proxy']['server-endpoint']['message']  + '</td>'
                htmlStart = htmlStart + '</tr></tbody></table>'
            else :
        ...
        if 'vpc-cni' in customer_report['addons'].keys() and len(customer_report['addons']['vpc-cni'].keys()) != 0 :
            if 'image' in customer_report['addons']['vpc-cni'] :
                htmlStart = htmlStart + '<p>VPC Cni </p>' + '<table><thead><td>Image</td><td>Mount Path</td><td>Version</td><td>Env</td></thead><tbody><tr>'
                ...
                if 'default-mountpaths' in customer_report['addons']['vpc-cni']['mount_paths'].keys() :
                    htmlStart = htmlStart + '<li>default-mountpaths ' + str(customer_report['addons']['vpc-cni']['mount_paths']['default-mountpaths']) + '</li>'
                if 'userdefined-mountpaths' in customer_report['addons']['vpc-cni']['mount_paths'].keys() :
                    htmlStart = htmlStart + '<li>userdefined-mountpaths ' + str(customer_report['addons']['vpc-cni']['mount_paths']['userdefined-mountpaths']) + '</li>'
                ...
                if 'default-envs' in customer_report['addons']['vpc-cni']['env'].keys() :
                    htmlStart = htmlStart + '<li>default-envs ' + str(customer_report['addons']['vpc-cni']['env']['default-envs']) + '</li>'
                if 'userdefined-envs' in customer_report['addons']['vpc-cni']['env'].keys() :
                    htmlStart = htmlStart + '<li>userdefined-envs ' + str(customer_report['addons']['vpc-cni']['env']['userdefined-envs']) + '</li>'
        ...
        if customer_report['nodegroup_details']:
            ...
            htmlStart = htmlStart + '<table><thead><tr><th>Node group type</th><th>Autoscaling group</th><th>Custom launch template</th><th>AMI image</th><th>Custom AMI</th><th>Node type</th><th>Version</th><th>Version Compatibility</th></tr></thead><tbody>'
            for k in node_groups.keys():
                if k == 'fargate' and len(node_groups[k].keys()) > 0:
                    for id in node_groups[k].keys():
                        htmlStart = htmlStart + '<tr><td>Fargate</td><td>--</td><td>--</td><td>--</td><td>--</td><td>' + id + '</td>' + '<td>' + node_groups[k][id]['version'] + '</td><td>' + str(node_groups[k][id]['version_compatibility']) + '</td></tr>'
                        ...
                        for n in node_groups[k][autoscaling]:
                            if n == 'instances' :
                                for inst in node_groups[k][autoscaling][n] :
                                    ...
                                    htmlStart = htmlStart + '<td>' + str(node_groups[k][autoscaling]["custom_launch_template"]) + '</td>'
