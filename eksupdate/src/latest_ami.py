...
def get_latestami(clustVersion,instancetype,image_to_search,region_Name):
    ...
    if "Amazon Linux 2" in instancetype:
        ...
        names=[
            '/aws/service/ami-windows-latest/{image_to_search}-{version}/image_id'.format(image_to_search=image_to_search,version=clustVersion)
        ]
    elif "bottlerocket" in instancetype.lower():
        names=[
            '/aws/service/bottlerocket/aws-k8s-{version}/x86_64/latest/image_id'.format(version=clustVersion)
        ]
    elif "Ubuntu" in instancetype:
        filters = [{'Name':'owner-id','Values':['099720109477']},{'Name': 'name', 'Values': ["ubuntu-eks/k8s_{version}*".format(version=clustVersion)]},{'Name':'is-public','Values':['true']}]
